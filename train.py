...
# Minimum number of matches a city/team must appear in to be kept.
MIN_CITY_MATCHES = 600
MIN_TEAM_MATCHES = 1500


def load_and_clean_data(path: str) -> pd.DataFrame:
    """Load the raw CSV and rename columns to the standard schema used by
    the model (batting_team, bowling_team, city, current_score, balls_left,
    wickets_left, crr, last_five, final_score)."""
    df = pd.read_csv(path)
    ...
    # Remove unrealistic rows (defensive cleaning).
...
def filter_rare_categories(df: pd.DataFrame) -> pd.DataFrame:
    """Filter out cities and teams that appear too rarely in the dataset,
    to keep the model's categorical vocabulary consistent."""
